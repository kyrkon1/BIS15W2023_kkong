...
```{r}
glimpse(fish) # mix of catagorical and numerical values (what is edge?) in ann number
```
...
```{r}
summary(fish) ## did 5 num sum for you
```
...
```{r}
select(fish, "lakeid", "scalelength") # select (data, variable "x", "y",) spelling counts, creating a new data frame
```
```{r}
select(fish, "lakeid", "annnumber") # typically put them in quotes 
```
...
```{r}
select(fish, fish_id:length) # pulls everything between fish id and length
```
...
```{r}
select(fish, contains("length")) # pulling out variables that contain
```
...
```{r}
select(fish, matches("a.+er")) # important to know matches a and ends with er
```
...
To select all columns that are *not* a class of data, you need to add a `~!` and (.).
```{r}
select_if(fish, ~!is.numeric(.))
```
...
```{r}
mean(mammals$`max. life`) # how do you get a negative mean  or mass (?)  they used the code -999 instead of a zero to indicate NA but R still thinks that -999 is a numeric, for ecological data -999 is a common code 
```
No NAs
...
```{r}
mammals_new <-rename(mammals, genus = "Genus", afr = "AFR", max_life = "max. life", litter_size = "litter size", litters_year = "litters/year")
names(mammals_new)
```
...
```{r}
select_if(mammals_new,~!is.numeric(.)) # the period is necessary for character class
```
```{r}
