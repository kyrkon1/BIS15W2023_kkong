...
heartrate3 <- heartrate2 %>% 
  separate(patient, into= c("patient", "sex"), sep = "_") %>% # taking patient into two new variables (patient and sex)
...
```{r}
tb_data %>% 
  pivot_wider(names_from = "key", #the observations under key will become new columns (new column names)
              values_from = "value")
```
...
gene_exp
```
it is not tidy, variable type has two variables in it "control" and "treatment"
```{r}
gene_exp %>% 
...
edu_level
```
Has BA and HS degree in the education leel
...
```{r}
beach_bugs <-read_csv(here("data","beachbugs_long.csv"))
beach_bugs
```

2. Use `pivot_wider` to transform the data into wide format.
```{r}
beach_bugs_wider <- beach_bugs %>% 
...
```{r}
beach_bugs_longer <- beach_bugs_wider %>% 
  pivot_longer(-c("year"),
               names_to = "site",
