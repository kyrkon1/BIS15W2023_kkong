...
```{r}
names(mammals) # renaming them for consistency 
```
...
```
## connecting the lines into one code
```{r}
fish %>% # fish shift cmd m or shift control m makes pipe, take the fish data and put it into the select pipe the results into lake id AL
...
```{r}
mammals %>%  ## take mammals data, choosing variables family, genus, species, gestation, and filtering out the gestation period of greater than or equal to 15 months
...
fish_subset <- fish %>% 
  ...
  filter(lakeid == "AL"| lakeid == "AR") %>%  # can do conditional
...
```{r}
mammals %>%
  mutate_all(tolower) # changing all column names to lowercase in one code
```
