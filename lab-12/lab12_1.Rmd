...
```{r, eval=F, echo=T}
runApp("plot_iris.R") # uses iris dataset, interactive: allows users to figure out what they want to plot (changing x and y axes)
```

## Shiny App Skeleton
A shiny app consists of a user interface (ui) function, server function, and run function.  

1. The `ui` function controls the user inputs and the way the app will be displayed.  (specify what parts are interactive)
...
shinyApp(ui, server) # run combines the ui and the server
```
...
server <- function(input, output, session) {
  output$plot<-renderPlot({
    ggplot(data= homerange, aes_string(x="locomotion", fill= input$x))+
      ...
      theme_light()+
      labs(x= NULL, y= NULL, fill = "Fill Variable") # null means there's no labels for the x or the y axis 
  })
