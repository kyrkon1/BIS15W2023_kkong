...
ui <- fluidPage(titlePanel("Homerange Locomotion"),
                ...
                plotOutput("plot", width="600px", height="500px") # raido button, you can click on which one to choose from
)
...
(with three interactions, selecting x, y variables and adjusting the point size)
```{r, eval=F, echo=T}
ui <- fluidPage(
...
server <- function(input, output, session) {
  session$onSessionEnded(stopApp) # when you close the session, stops the app 
  output$plot <- renderPlot({
