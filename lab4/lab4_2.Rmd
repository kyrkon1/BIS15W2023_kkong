...
select takes out columns, filter looks in rows 
## `filter()`
...
```{r}
filter(fish, lakeid == "AL") # filter (data, x == (equal to) y)
```
...
```{r}
filter(fish, length %in% c(167, 175)) # filter within a range look within (%in%) to find lengths between 167 and 175
```
...
```{r}
filter(fish, near(radii_length_mm, 2, tol = 0.2)) # tol is tolerance
```
...
```{r}
filter(fish, lakeid == "AL" & length > 350) # filter where lake id is AL and the length is greater than 350
```

Notice that the `|` operator generates a different result.
```{r}
filter(fish, lakeid == "AL" | length > 350) # or operator | either all lake id is equal to AL or the length is greater than 350
```
...
mammals_new <- select(mammals,family:gestation)
mammals_new
```
...
```{r}
select(mammals, family, mass, gestation, newborn)
newframemammal <- select(mammals, family, mass, gestation, newborn)
