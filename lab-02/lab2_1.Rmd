...
x
```
## if you are confused about what x is, can type x into the code
```{r}
y <- 5
```
...
my_experiment
```
## making a new object in environment, sum (object 1, object 2)

## Nomenclature
We need to be careful about nomenclature when we write code. R allows us to give almost any name we want to an object, but there are exceptions. For example, we don't want to give a name to an object that is the same as a function in R (creates problems)
...
```{r}
speeds <- c(270,261,268) #making new object, combining all the cars (catatinate)
mean(speeds)

```
## if you had typed mean(venom_GT, chiron, veyron), you'd get the wrong answer. 

```{r}
cars_2 <- c(venom_GT, chiron, veyron) # can use object labels instead of values
mean(cars_2)
```
...
```{r}
my_numeric <- 42 # any numerical value
my_integer <- 2L #adding an L automatically denotes an integer
my_character <- "universe"
my_logical <- FALSE # true or false
my_complex <- 2+4i #formula
```
...
```
## can change something into an integer, making a new object. changing numeric
```{r}
my_integer <- 
...
my_missing <- NA
```
## is na missing values, predict true
```{r}
is.na(my_missing)
```
...
NA, it means that since there is data missing, the calculation of the mean is inconclusive. The NAs are significant and must be considered when conducting statistical analyses. It is exceedingly difficult to deal with this.

4. Recalculate the mean using the following code chunk. Why is the useful?
## remove the NA, then calculates the mean
```{r}
mean(new_vector, na.rm=T) #na.rm removes the NA values in the vector, T for True
```
This is useful to find the mean of the data set (minus the missing values or NA)
