...
## later on working with data frames aka spreadsheets
...
```{r}
my_vector <- c(10, 20, 30) # vector gets catatination of 10,20,30
```
#num says numeric and 1:3, one row and 3 columns
A character vector. Characters always have quotes and may be referred to as "strings".
## chr says character
```{r}
days_of_the_week <- c("Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday") # quotations tell us that they are characters and not objects
```
...
Box office earnings for Harry Potter movies (in millions!). Notice that these are separate vectors.  U.S and International
```{r}
...
```{r}
harry_potter_matrix <- matrix(box_office, nrow = 8, byrow = T) # making new object, new matrix and 8 rows and organized by rows
harry_potter_matrix # stacked vector
```
## Name the rows and columns
Vectors `region` and `titles`, used for naming.
```{r}
region <- c("US", "non-US") # new vectors
region
```
...
```{r}
total_earnings <- colSums(harry_potter_matrix,global)
total_earnings
```

2. Add this information to the data matrix. Hint: you are adding a row, not a column.  
```{r}
final_harry_potter_matrix <- rbind(all_harry_potter_matrix, total_earnings) #new name
