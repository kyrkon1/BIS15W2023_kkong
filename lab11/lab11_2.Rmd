...
p+theme_linedraw()+
  ...
  labs(title = "Observations by Taxon in Homerange Data",
       x = NULL,
       y= "n",
       fill= "Trophic Guild") #generating reports, better if plots are consistent
```
...
```{r}
display.brewer.pal(4,"Paired") # number of colors, and then the palettes, always be concerned for color blindness
```
...
+`scale_colour_brewer()` is for points  for scatterplot
+`scale_fill_brewer()` is for fills  for barplot
...
```{r}
my_palette <- paletteer_d("beyonce::X42") # package :: name
```
...
```
```{r}
p2+ scale_color_brewer(palette= "") #scale color brewer for scatterplot
```
...
```{r}
ecosphere %>% 
  ggplot(aes(x=diet, y=log10_mass, fill=migratory_strategy))+  # two categorical variables
  geom_boxplot(alpha=0.4)
```
...
ecosphere %>% 
  ggplot(aes(x=diet, y=log10_mass, fill=migratory_strategy))+ 
  geom_boxplot(alpha=0.4)+ 
  facet_grid(migratory_strategy~.)+ # a (rows) b is columns, not specified columns so just rows
  theme(axis.text.x = element_text(angle = 60, hjust = 1))+
  labs(title = "Mass by Diet and Migratory Strategy",
       x = NULL,
       y = "Log 10 Mass",
