...
```{r}
map1 <- get_map(bbox, maptype = "terrain", source = "stamen") #use stamen bc its free
```
...
```{r}
ggmap(map1) +  #ggmap with map 1, plus geom point (points for the spiders), aesthetics are longitude and latitude
...
long2 <- c(-166.2, -131.3)
lat2<-c(55.01,70.36)
cbox<-make_bbox(long2,lat2, f =0.05)
```

2. Load a map from `stamen` in a terrain style projection and display the map.
```{r}
map2<-get_map(cbox, maptype= "terrain", source = "stamen")
```
...
```{r}
ggmap(map2) +
  geom_point(data= grizzly, aes(x=longitude, y= latitude, alpha =0.5))+
  labs(title = "Recorded Observations of Grizzly Bears in Alaska", x= "Longitude", y= "Latitude") # or bear type Ursos Arctos
```
