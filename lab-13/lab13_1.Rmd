...
```{r, message = FALSE}
spider_list <- lapply(files, read_csv) # makes a list in files, read.csv (iterates all of the files) from the list of files made earlier
```
...
```{r}
inner_exampleDF <- inner_join(table_A, table_B, by="customer_ID") #only focused on the same two customer IDs, joining between the two tables (common customer ids in both table a and b, where all observations for both things you are joining are the same
inner_exampleDF
```
...
```{r}
left_exampleDF <- left_join(table_A, table_B, by="customer_ID") #merged the table A to table B, but since there is no data for customer 3 for the Table B, it is NA. Customer 4 goes away because it joins only TABLE A
left_exampleDF
```
...
`full_join`
```{r}
full_exampleDF <- full_join(table_A, table_B, by="customer_ID") # easier to deal with missing data than by using the other types of join (left or right)
full_exampleDF
```
...
```{r}
spiders_with_locs <- 
  left_join(spiders_all, spiders_locs, by = "Accession") # joining the tables with the location, making one to one matches
...
```{r}
class(spiders_with_locs$Date) # R makes its best guess as to what the date is, for this date is interpreted as a character
